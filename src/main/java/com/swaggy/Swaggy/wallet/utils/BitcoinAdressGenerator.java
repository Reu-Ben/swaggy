package com.swaggy.Swaggy.wallet.utils;
...
@Component
public class BitcoinAdressGenerator {
	...
	/*************
	 * This method will create the ECDSA public and private key which is returned in
	 * a Byte array of Byte arrays, index 0 is the private key and index 1 of the
	 * array will return the ECDSA public key
	 * 
	 * @return
	 * 
	 */
	private byte[][] ECDSAgeneratePublicAndPrivateKey() {
		...
		System.arraycopy(dd.getYCoord().toBigInteger().toByteArray(), 0, publickey,
				64 - dd.getYCoord().toBigInteger().toByteArray().length + 1,
				dd.getYCoord().toBigInteger().toByteArray().length);
		System.arraycopy(dd.getXCoord().toBigInteger().toByteArray(), 0, publickey,
				32 - dd.getXCoord().toBigInteger().toByteArray().length + 1,
				dd.getXCoord().toBigInteger().toByteArray().length);
		publickey[0] = 4;
	...
	/********
	 * This method does all the RipeMD 160 hashing function, and after that hash has
	 * taken place, it prepends the version byte to the beginning of the Byte array
	 * 
	 ...
	 * @param secondarray - this is the second array that will be placed in the
	 *                    empty array
	 * @return - returns one byte array which is the concate of the first and second
	 *         byte array
	 */
	...
	/********
	 * Another helper function that will convert a byte array to a Hex String with
	...
	/***************
	 * This encodes a String input to a base 58 taken from the bitcoinj
	 * implmentation of bas58checking encoding.
	 * 
...
// Add as many leading '1' as there were leading zeros.
	...
	/*********
	 * This method takes a byte array of the private key and returns a String of a
	 * value Wallet Import Formatted Private key to be able to import it to a wallet
	 * 
	 * @param privatekey - the byte array of the private key you want to make wallet
	 *                   import format
	...
	/**********
	 * This method will return a string array of the 3 keys. The first element is
	 * the private key, the second element is the private key in Wallet Injection
	 * Format, the third element is the bitcoin address.
	 * 
	 * @return
	 */
